...
def test_user_interaction(test_list_hh, capsys):
    ...
    assert captured.out == "Ошибка преобразования вакансии в объект: Ожидается список словарей\n"
...
def test_get_vacancies_by_salary(test_list_hh, capsys):
    ...
    assert repr(ranged_vacancies) == (
        "[{'id': 118832907, 'name': 'Вакуумщик/Оператор вакуумного станка', 'area': "
    ...
    captured = capsys.readouterr()
    assert captured.out == (
        ("Ошибка при попытке получения выборки вакансий: invalid literal for int() " "with base 10: 'sdfs'\n")
    )
...
def test_sort_vacancies(test_list_hh, capsys):
    ...
    captured = capsys.readouterr()
    assert captured.out == ("Ошибка сортировки <class 'Exception'>: Проверьте данные для направления " "сортировки\n")
